...
impl DrawL3d {
    ...
    pub fn create(canvas: web_sys::HtmlCanvasElement) -> Result<Self, String> {
        #[cfg(target_arch = "wasm32")]
        {
            ...
            log!(format!("[Renderer-{}] Creating with canvas size: {}x{}", id, width, height));
    ...
    pub fn set_model(&mut self, l3d: &l3d_rs::L3d) {
        ...
        log!(format!("[Renderer-{}] L3D model parts: {}", self.id, l3d.model.parts.len()));
        log!(format!("[Renderer-{}] L3D file assets: {}", self.id, l3d.file.assets.len()));

        // Log all available asset names
        let asset_names: Vec<_> = l3d.file.assets.iter().map(|a| a.name.as_str()).collect();
        log!(format!("[Renderer-{}] Available assets: {:?}", self.id, asset_names));

        // Log asset details
        for (i, asset) in l3d.file.assets.iter().enumerate() {
            log!(format!("[Renderer-{}] Asset {}: name='{}', content={} bytes", self.id, i, asset.name, asset.content.len()));
        }

        // Log part info
        for (i, part) in l3d.model.parts.iter().enumerate() {
            log!(format!("[Renderer-{}] Part {}: path='{}', looking for asset match", self.id, i, part.path));
            // Check if asset exists with this exact name
            let asset_exists = l3d.file.assets.iter().any(|a| a.name == part.path);
            log!(format!("[Renderer-{}] Part {} asset match: {}", self.id, i, asset_exists));
        }

        if l3d.model.parts.is_empty() {
            log!(format!("[Renderer-{}] WARNING: No model parts found!", self.id));
            return;
        }

        let mut loaded = load_assets(l3d);
        log!(format!("[Renderer-{}] RawAssets created, starting model loading...", self.id));
        ...
        log!(format!("[Renderer-{}] Loading {} model parts", self.id, l3d.model.parts.len()));

        for (part_idx, part) in l3d.model.parts.iter().enumerate() {
            log!(format!("[Renderer-{}] Processing part {}: path='{}'", self.id, part_idx, part.path));

            let cpu_mdl = match cpu_models.get(&part.path) {
                Some(m) => {
                    log!(format!("[Renderer-{}] Using cached CpuModel for '{}'", self.id, part.path));
                    m
                },
                None => {
                    log!(format!("[Renderer-{}] Deserializing CpuModel for '{}'...", self.id, part.path));
                    match loaded.deserialize::<CpuModel>(&part.path) {
                        Ok(m) => {
                            log!(format!("[Renderer-{}] Successfully deserialized '{}'", self.id, part.path));
                            cpu_models.insert(&part.path, m);
                            cpu_models.get(&part.path).unwrap()
                        }
                        Err(e) => {
                            log!(format!("[Renderer-{}] FAILED to deserialize model '{}': {:?}", self.id, &part.path, e));
                            continue;
                        }
                    }
                },
            ...
            log!(format!("[Renderer-{}] Creating GPU model for '{}'...", self.id, part.path));
            match Model::<PhysicalMaterial>::new(&self.context, cpu_mdl) {
                Ok(mut mdl) => {
                    log!(format!("[Renderer-{}] GPU model created successfully for '{}'", self.id, part.path));
        ...
        log!(format!("[Renderer-{}] set_model complete: loaded {} GPU models from {} parts",
            self.id, models.len(), l3d.model.parts.len()));
        self.models = models;
        ...
        if min_bound.x < max_bound.x {
            ...
            log!(format!("Model bounds: {:?} to {:?}, center: {:?}, distance: {}",
    ...
    pub fn orbit(&mut self, delta: (f32, f32)) {
        let event = Event::MouseMotion {
            ...
            modifiers: Modifiers { alt: false, ctrl: false, shift: false, command: false },
            handled: false,
        };
    ...
    pub fn render(&mut self) {
        ...
        if self.frame_count == 1 || self.frame_count % 60 == 0 {
            log!(format!("[Renderer-{}] render() frame {}, models: {}", self.id, self.frame_count, self.models.len()));
        }
            ...
            if let Ok(content) = std::str::from_utf8(&asset.content) {
                ...
                for line in content.lines() {
                    ...
                    if line.starts_with("mtllib ") {
                        ...
                        log!(format!("[load_assets] OBJ '{}' references mtllib '{}'", asset.name, mtl_name));
