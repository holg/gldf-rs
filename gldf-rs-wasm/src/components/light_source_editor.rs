...
pub fn light_source_editor() -> Html {
    ...
    let fixed_count = light_sources.map(|ls| ls.fixed_light_source.len()).unwrap_or(0);
    let changeable_count = light_sources.map(|ls| ls.changeable_light_source.len()).unwrap_or(0);
    ...
    html! {
        <div class="editor-section light-source-editor">
            ...
            <div class="tabs">
                <button
                    class={classes!("tab", (*active_tab == "fixed").then(|| "active"))}
                    onclick={{
                ...
                <button
                    class={classes!("tab", (*active_tab == "changeable").then(|| "active"))}
                    onclick={{
                        let on_tab_click = on_tab_click.clone();
...
fn fixed_light_sources_panel() -> Html {
    let gldf = use_gldf();
    let fixed_sources = gldf.product.general_definitions.light_sources
        .as_ref()
...
fn changeable_light_sources_panel() -> Html {
    let gldf = use_gldf();
    let changeable_sources = gldf.product.general_definitions.light_sources
        .as_ref()
