...
pub enum GldfAction {
    ...
    AddFile { id: String, content_type: String, type_attr: String, file_name: String, language: Option<String> },
    /// Remove a file by id
    RemoveFile(String),
    /// Update a file's properties
    UpdateFile { id: String, content_type: String, type_attr: String, file_name: String },
...
impl Reducible for GldfState {
    ...
    fn reduce(self: Rc<Self>, action: Self::Action) -> Rc<Self> {
        ...
        match action {
            ...
            GldfAction::SetFormatVersion { major, minor, pre_release } => {
            ...
            GldfAction::AddFile { id, content_type, type_attr, file_name, language } => {
            ...
            GldfAction::RemoveFile(id) => {
                new_state.product.general_definitions.files.file.retain(|f| f.id != id);
            }
            GldfAction::UpdateFile { id, content_type, type_attr, file_name } => {
                if let Some(file) = new_state.product.general_definitions.files.file.iter_mut().find(|f| f.id == id) {
...
pub fn use_gldf() -> GldfContext {
    use_context::<GldfContext>().expect("GldfContext not found. Did you wrap your component in GldfProvider?")
}
