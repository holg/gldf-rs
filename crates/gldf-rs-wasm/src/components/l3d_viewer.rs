...
pub fn l3d_viewer(props: &L3dViewerProps) -> Html {
    ...
    log!(format!(
        "[L3D-{}] Component rendering, data size: {} bytes",
        *viewer_id,
        props.l3d_data.len()
    ));
    ...
    // Initialize renderer when canvas is mounted AND we have data
    {
        ...
        use_effect_with(data_len, move |_| {
            log!(format!(
                "[L3D-{}] Effect triggered, data_len: {}, initialized: {}",
                viewer_id, data_len, *initialized
            ));
                    ...
                    if let Some(canvas) = canvas_ref.cast::<web_sys::HtmlCanvasElement>() {
                        ...
                        if !l3d.model.parts.is_empty() {
                            ...
                            match DrawL3d::create(canvas) {
                                Ok(mut draw_l3d) => {
                                    log!(format!(
                                        "[L3D-{}] DrawL3d created successfully",
                                        viewer_id
                                    ));
                            ...
                            log!(format!(
                                "[L3D-{}] No model parts found in L3D file",
                                viewer_id
                            ));
                        }
                    } else {
